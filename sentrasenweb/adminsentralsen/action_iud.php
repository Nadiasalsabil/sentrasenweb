...
//INSERT MASTER TARIF
else if(isset($_POST['sbmt_tarif']) && $_POST['dikirimdlm'] != ''){
	
	$dari = $_POST['dari'];
	$ke = $_POST['ke'];
	$layanan = $_POST['layanan'];
	$dikirimdlm = $_POST['dikirimdlm'];
	$minperkg = $_POST['minperkg'];
	$tarif = $_POST['tarif'];
	$via = $_POST['via'];
	
	$query = "INSERT INTO msttarif(dari,ke,layanan,dikirimdlm,minperkg,tarif,via,createddate) VALUES 
	('$dari','$ke','$layanan','$dikirimdlm','$minperkg','$tarif','$via',now())";
	$exe_query = $use_pdo -> prepare($query);
	$exe_query -> execute();
		
	if($exe_query){	
	echo '<script>
		alert("input master tarif berhasil");
		location.reload();
		</script>';
	}else{
	echo '<script>
		alert("input master tarif gagal");
		location.reload();
		</script>';		
	}
}
...
//INSERT TRANSAKSI CUSTOMER 
else if(isset($_POST['sbmt_pengirimancustomerupdate'])){
	//echo "<script>alert('sukses ya');</script>";
	//$user_id = $_SESSION['user_session'];
	$notransc 		= $_POST['notransc'];
	...
	$query = "UPDATE transpengiriman_cust set
								beratbrg	='$berat',
								jumbrg		='$jumbrg',
								tarif		='$tarif',
								total		='$total',
								sts			='$sts',
								deskripsibrg='$deskbrg',
								tglpeng		='$tglpengiriman',
								waktupeng	='$waktupengbrg',
								kotaasal	='$kotaasal',
								kotatujuan	='$kotatujuan',
								areaasal	='$areaasal',
								detailtujuan='$detailtujuan',
								lokasipeng	='$lokasipeng',
								service		='$service',
								payment		='$payment',
								tipebarang	='$tipebrg',
								catatan		='$catatan'
	...
	echo '<script>
		...
		window.location.href = "transaksi_pengiriman";
		</script>';
	}else{
	echo '<script>
		alert("update gagal");
...
//UPDATE MASTER TARIF
else if(isset($_POST['updt_tarif']) && $_POST['updikirimdlm'] != ''){
	
	$token = $_POST['token'];
	$updari = $_POST['updari'];
	$upke = $_POST['upke'];
	$uplayanan = $_POST['uplayanan'];
	$updikirimdlm = $_POST['updikirimdlm'];
	$upminperkg = $_POST['upminperkg'];
	$uptarif = $_POST['uptarif'];
	$upvia = $_POST['upvia'];
	
	$query = "
	UPDATE msttarif SET dari='$updari',ke='$upke',layanan='$uplayanan',dikirimdlm='$updikirimdlm',
	minperkg='$upminperkg',tarif='$uptarif',via='$upvia',modifieddate=now() WHERE notarif='$token'";
	$exe_query = $use_pdo -> prepare($query);
	$exe_query -> execute();
		
	if($exe_query){	
	echo '<script>
		alert("update master tarif berhasil");
		location.reload();
		</script>';
	}else{
	echo '<script>
		alert("update master tarif gagal");
		location.reload();
		</script>';		
	}
}
...
//DELETE TARIF
else if(isset($_POST['del_tarif']) && $_POST['token'] != ''){
	
	$token = $_POST['token'];
	
	$query = "DELETE FROM msttarif WHERE notarif = '$token'";
	$exe_query = $use_pdo -> prepare($query);
	$exe_query -> execute();
		
	if($exe_query){	
	echo '<script>
		alert("delete tarif berhasil");
		//location.reload();
		</script>';
	}else{
	echo '<script>
		alert("delete tarif gagal");
		//location.reload();
		</script>';		
	}
}
